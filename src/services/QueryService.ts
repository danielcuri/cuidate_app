...
export class QueryService {
  async executeQuery<T>(type: string, query: string, data: unknown): Promise<T> {
  ...
  async executeQueryLearning<T>(type: string, query: string, data: unknown): Promise<T> {
    const method = type.toUpperCase() as Method;
  ...
  async executeQueryMedical<T>(type: string, query: string, data: unknown): Promise<T> {
    const method = type.toUpperCase() as Method;
    const config: AxiosRequestConfig = { method, url: query };
    if (method === 'GET') {
      config.params = data;
