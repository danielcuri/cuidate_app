...
type CanvasRoute = RouteProp<
    RootStackParamList,
    "CanvasForm" | "CanvasFormEdit"
>;
type CanvasNav = StackNavigationProp<
    RootStackParamList,
    "CanvasForm" | "CanvasFormEdit"
>;
...
export function CanvasForm() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (fid == null) {
                setErr(
                    "No hay formularios cargados. Actualice desde el menú Formularios.",
                );
                setForm(null);
                setFormData(null);
                return;
            ...
            if (
                draftIndex != null &&
                formService.forms_saved_local[draftIndex]
            ) {
                const raw = formService.forms_saved_local[
                    draftIndex
                ] as FormDataShape;
                setFormData({
                    id: raw.id ?? "",
                    ...
                    form_data_ids: {
                        ...(raw.form_data_ids as Record<number, unknown>),
                    },
    ...
    const appendPhoto = async (fieldId: number) => {
        try {
            ...
            alertService.present(
                "Foto",
                "No se pudo abrir la cámara. Revise permisos de cámara/medios.",
            );
        }
    };
    ...
    const appendPhotoFromGallery = async (fieldId: number) => {
        try {
            ...
            alertService.present(
                "Galería",
                "No se pudo abrir la galería. Revise permisos de medios.",
            );
        }
    };
        ...
        return (
            <View style={styles.page}>
                <SafeAreaView style={styles.safeTop} edges={["top"]}>
                    <RecordsHeader
                        title={String(form?.name ?? "Formulario")}
                ...
                <View style={styles.navRow}>
                    ...
                    >
                        <Text
                            style={[
                                styles.navBtnTxt,
                                slideIndex === 0 && styles.navBtnTxtDim,
                            ]}
                        >
                            Anterior
                        </Text>
