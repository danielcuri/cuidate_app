...
export function Formats() {
  ...
  const formatsList = useMemo(() => [...forms, ...(showPamolsa ? [pamolsaItem] : [])], [forms, showPamolsa, pamolsaItem]);
  ...
  return (
    <View style={styles.page}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={[styles.row, (item as any)?.isStatic ? styles.rowStatic : null]}
            onPress={() => open(item as Forms & { isStatic?: boolean })}
            activeOpacity={0.88}
          >
            <View style={{ flex: 1 }}>
              ...
              {(item as any)?.isStatic ? <Text style={styles.rowMeta}>Pamolsa · Hallazgo SST</Text> : null}
            </View>
