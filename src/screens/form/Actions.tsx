...
export function Actions() {
  ...
  const load = useCallback(
    async (reset: boolean) => {
      ...
      const res = await formService.getListActions(payload);
      const chunk = extractList<Action>(res, ['actions', 'data']);
  ...
  return (
    <View style={styles.page}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <FormListCard
            title={item.proposed_actions ?? `Acción #${item.id}`}
            subtitle={item.findings}
            meta={`${item.proposed_date ?? ''} · Riesgo: ${item.risk_level ?? '—'}`}
